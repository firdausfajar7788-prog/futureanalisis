...
def analyze_mtf_macd_stoch(symbol, timeframes=["15m", "1h", "4h"]):
    ...
    combined["total_score"] = total_score  # <--- TAMBAHKAN
    combined["confirmations"] = buy_count + sell_count  # <--- TAMBAHKAN
    combined["smart_money"] = {"score": 50}  # <--- TAMBAHKAN (default)
    
    # Ambil trend dari timeframe 1h dan 15m
...
with tab1:
    ...
    for idx, symbol in enumerate(st.session_state.watchlist[:50]):
        ...
        if result:
            ...
            # ========== INI DIPINDAH KE DALAM LOOP (sejajar dengan if result) ==========
